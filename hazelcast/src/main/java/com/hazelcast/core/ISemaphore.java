 ...
 * {@link #acquire() acquire} methods are selected to obtain permits in the order in which their
 ...
 * it is possible for one member to invoke {@code acquire} before another, but reach the ordering
 * point after the other, and similarly upon return from the method.
 * <p>
 * This class also provides convenience methods to {@link #acquire(int) acquire} and
 * {@link #release(int) release} multiple permits at a time. Beware of the increased risk of
...
public interface ISemaphore extends DistributedObject {
     ...
     * <li>some other thread invokes one of the {@link #release() release}
     * methods for this semaphore, the current thread is next to be assigned
     * permits and the number of available permits satisfies this request, or</li>
     * <li>some other thread {@linkplain Thread#interrupt interrupts}
     * the current thread, or</li>
     * <li>the specified waiting time elapses.</li>
     * </ul>
