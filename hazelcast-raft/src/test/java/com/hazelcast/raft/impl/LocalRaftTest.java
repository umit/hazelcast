...
public class LocalRaftTest extends HazelcastTestSupport {
    ...
    public void when_commitLogAdvances_then_snapshotIsTaken() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < entryCount; i++) {
            leader.replicate(new RaftApplyOperation("val" + i)).get();
        }
    ...
    public void when_followerMissesTheLastEntryThatGoesIntoTheSnapshot_then_itInstallsSnapshot() throws ExecutionException, InterruptedException {
        ...
        leader.replicate(new RaftApplyOperation("val" + (entryCount - 1))).get();
    ...
    public void when_isolatedLeaderAppendsEntries_then_itInvalidatesTheirFeaturesUponInstallSnapshot() throws ExecutionException, InterruptedException {
        ...
        for (int i = 40; i < 45; i++) {
            Future f = leader.replicate(new RaftApplyOperation("isolated" + i));
            futures.add(f);
        }
    ...
    public void when_terminateOpIsAppended_then_statusIsTerminating() {
        ...
        final RaftNode leader = group.waitUntilLeaderElected();
        final RaftNode follower = group.getAnyFollowerNode();
    ...
    public void when_leaderFailsWhileLeavingRaftGroup_othersCommitTheMemberChange() throws ExecutionException, InterruptedException {
        ...
        final RaftNode newLeader = group.getNode(getLeaderEndpoint(followers[0]));
        newLeader.replicate(new RaftApplyOperation("val2"));
    ...
    static LocalRaftGroup newGroupWithService(int nodeCount, RaftConfig raftConfig) {
        return new LocalRaftGroup(nodeCount, raftConfig, SERVICE_NAME, RaftDataService.class);
    }

    static RaftConfig newRaftConfigWithNoSnapshotting() {
